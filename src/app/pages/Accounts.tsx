...
// Detect login method from email
function getLoginMethod(email: string): { label: string; color: string; icon: React.ReactNode } {
  ...
  if (email.includes('github') || email.includes('@users.noreply.github')) {
    return {
      label: 'GitHub',
  ...
  if (email.includes('@icloud') || email.includes('@me.com') || email.includes('@mac.com')) {
    return {
      label: 'Apple',
  ...
  const emailStats = useMemo(() => {
    ...
    return Object.entries(grouped)
      .map(([email, subs]) => {
        ...
        const loginMethod = getLoginMethod(email);
